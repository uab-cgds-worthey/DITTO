...
for samples in json_file["train"].keys():
    if "PROBAND" in samples:
        ...
        for i in range(len(json_file["train"][samples]["solves"])):
            ...
            all_files = glob.glob(
                os.path.join(
                    f"/data/project/worthey_lab/projects/experimental_pipelines/mana/small_tasks/cagi6/rgp/data/processed/exomiser/hpo_original/train/{samples}",
                    "*.tsv",
                )
            )
            ...
            exo_scores = exo_scores.sort_values(
                "EXOMISER_GENE_PHENO_SCORE", ascending=False
            )
            exo_scores = exo_scores.drop_duplicates(
            ...
            with open(f"{args.input_dir}/{args.output}", "a") as f:
                ...
                f.write(
                    f"{samples},{gene},{exo_scores.loc[rank-1]['EXOMISER_GENE_PHENO_SCORE'].values},{rank.tolist()}\n"
                )
