...
with open("annotated_vcf_ranks.csv", 'w') as f:
...
for samples in json_file['train'].keys():
        if "PROBAND" in samples and "TRAIN" in samples:
            genes = pd.read_csv(f"/data/project/worthey_lab/projects/experimental_pipelines/tarun/ditto/data/processed/annotated_vcf/train/{samples}/combined_predictions.csv")#, sep=':')
            ...
            for i in range(len(json_file['train'][samples]["solves"])):
                ...
                with open("annotated_vcf_ranks.csv", 'a') as f:
