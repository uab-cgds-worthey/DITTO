...
def extract_col(config_dict,df, stats):
    ...
    df=df[(df['Alternate Allele'].str.len() < 2)]
    df=df[(df['Reference Allele'].str.len() < 2)]
    ...
    df.dropna(axis=0, thresh=(df.shape[1]*0.3), inplace=True)  #thresh=(df.shape[1]/1.2)    how='all',
...
def fill_na(df,config_dict): #(config_dict,df):
    #df1  = pd.DataFrame()
    var = df[config_dict['snps']]
    df = df.drop(config_dict['snps'], axis=1)
...
def main(var_f, config_f, stats):
    ...
    df.isnull().sum(axis = 0).to_csv('./data/processed/NA-counts-snp.csv')
    ...
    df = fill_na(df,config_dict) #(config_dict,df)
    ...
    df.to_csv('./data/processed/merged_data-snp.csv', index=False)
    y.to_csv('./data/processed/merged_data-y-snp.csv', index=False)
    return None
...
if __name__ == "__main__":
    ...
    stats = "./data/processed/stats-snp.csv"
