...
def get_roc_plot(so, X_test, Y_test, outdir, weights):
    ...
    for name in list(X_test.columns):
        ...
        auc = roc_auc_score(
            Y_test, X_test[name].round(), sample_weight=weights, average="weighted"
        )
        auc = "{:.2f}".format(auc)
        roc_scores_so[name] = auc
        ax_roc.plot(fpr, tpr, label=str(name) + " = " + str(auc))
        precision, recall, _ = precision_recall_curve(
            Y_test, X_test[name].round(), sample_weight=weights
        )
...
def get_matrix(y_score, Y_test, so, outdir):
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm, display_labels=["Path", "Benign"]
    )
    disp.plot()
...
def get_SHAP(test_x, clf, so, outdir, feature_names):
    ...
    explainer = shap.KernelExplainer(
        model=clf, data=background_x
    )  # (background_so, background_x))
...
def run_test(X_test, outdir, clf, config_dict, feature_names, class_weights):
    ...
    for so in consequence_list:
        ...
        if not test_x.empty and len(Y_test.unique()) == 2:
            ...
            benchmark_df.columns = benchmark_df.columns.to_series().map(
                config_dict["Benchmark_cols"]
            )
            ...
            roc_scores_so, prc_scores_so, f1_scores_so = get_roc_plot(
                so, benchmark_df, Y_test, outdir, class_weights
            )
...
def data_parsing(test_x, config_dict):
    ...
    Y_test = label_binarize(Y_test.values, classes=list(np.unique(Y_test))).ravel()
    X_test["class"] = Y_test
    print("Data Loaded!")
...
if __name__ == "__main__":
    ...
    clf = load_model(ARGS.ditto)
    X_test, feature_names, class_weights = data_parsing(ARGS.test_x, config_dict)
