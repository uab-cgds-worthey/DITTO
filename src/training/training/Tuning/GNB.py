...
class stacking(
    ...
    def setup(self, config, x_train, x_test, y_train, y_test):
        ...
        self.model = GaussianNB(var_smoothing=self.config.get("var_smoothing", 1e-09))

    def reset_config(self, new_config):
        score = cross_validate(
            self.model, self.x_train, self.y_train, cv=10, n_jobs=-1, verbose=0
        )
    ...
    def step(self):
        score = cross_validate(
            self.model, self.x_train, self.y_train, cv=5, n_jobs=-1, verbose=0
        )
...
def results(config, x_train, x_test, y_train, y_test, var, output, feature_names):
    ...
    clf_name = "GaussianNB"
    clf = GaussianNB(var_smoothing=config.get("var_smoothing", 1e-09))
...
if __name__ == "__main__":
    ...
    config = {"var_smoothing": hp.loguniform("var_smoothing", 1e-11, 1e-1)}
