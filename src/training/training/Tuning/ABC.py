...
class stacking(
    ...
    def setup(self, config, x_train, x_test, y_train, y_test):
        ...
        self.model = AdaBoostClassifier(
            algorithm=self.config.get("algorithm", "SAMME"),
            base_estimator=DecisionTreeClassifier(
                max_depth=self.config.get("max_depth", 1)
            ),
            learning_rate=self.config.get("learning_rate", 1),
            n_estimators=self.config.get("n_estimators", 5),
        )
    ...
    def step(self):
        score = cross_validate(
            self.model, self.x_train, self.y_train, cv=10, n_jobs=20, verbose=0
        )
...
def results(config, x_train, x_test, y_train, y_test, var, output, feature_names):
    ...
    clf = AdaBoostClassifier(
        algorithm=config.get("algorithm", "SAMME"),
        base_estimator=DecisionTreeClassifier(max_depth=config.get("max_depth", 1)),
    ...
    print(
        f"Model\ttrain_score\tPrecision\tRecall\troc_auc\tAccuracy\tTime(min)\tConfusion_matrix[low_impact, high_impact]",
        file=open(output, "a"),
...
if __name__ == "__main__":
    ...
    print(
        f"AdaBoost_{var}:  {config}", file=open(f"../tuning/tuned_parameters.csv", "a")
    )
