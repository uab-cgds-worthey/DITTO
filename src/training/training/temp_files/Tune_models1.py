...
@ray.remote
def classifier(
    clf, model, var, X_train, X_test, Y_train, Y_test, feature_names, output
):
    os.chdir(
        "/data/project/worthey_lab/projects/experimental_pipelines/tarun/ditto/data/processed/"
    )
...
def tuning(models, var, X_train, X_test, Y_train, Y_test, feature_names, output):
    ...
    for model, config in models.items():
        ...
        skopt_tune_search = TuneSearchCV(
            model,
        ...
        print(
            f"{model}_{var}:{skopt_tune_search.best_params_}",
...
if __name__ == "__main__":
    ...
    for var in variants:
        ...
        for model in classifiers:
            ray.get(
                tuning.remote(
                    model, var, X_train, X_test, Y_train, Y_test, feature_names, output
                )
            )
