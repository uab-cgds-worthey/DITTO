...
def data_parsing(var,config_dict,output):
    ...
    X_train = pd.read_csv(f'train_AF_0_F_50_{var}/merged_data-train_AF_0_F_50_{var}.csv')
    #var = X_train[config_dict['ML_VAR']]
    X_train = X_train.drop(config_dict['ML_VAR'], axis=1)
    feature_names = X_train.columns.tolist()
    X_train = X_train.values
    Y_train = pd.read_csv(f'train_AF_0_F_50_{var}/merged_data-y-train_AF_0_F_50_{var}.csv')
    Y_train = label_binarize(Y_train.values, classes=['low_impact', 'high_impact']).ravel() 

    X_test = pd.read_csv(f'test_AF_0_F_50_{var}/merged_data-test_AF_0_F_50_{var}.csv')
    #var = X_test[config_dict['ML_VAR']]
    X_test = X_test.drop(config_dict['ML_VAR'], axis=1)
    #feature_names = X_test.columns.tolist()
    X_test = X_test.values
    Y_test = pd.read_csv(f'test_AF_0_F_50_{var}/merged_data-y-test_AF_0_F_50_{var}.csv')
    print('Data Loaded!')
    #Y = pd.get_dummies(y)
...
def classifier(clf,var, X_train, X_test, Y_train, Y_test, background,feature_names, output):
   ...
   with open(f"./models/AF_0_F_50_{var}/{clf_name}_{var}.joblib", 'wb') as f:
   ...
   plt.savefig(f"./models/AF_0_F_50_{var}/{clf_name}_{var}_features.pdf", format='pdf', dpi=1000, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    var = 'nssnv'
    if not os.path.exists('models/AF_0_F_50_'+var):
        os.makedirs('./models/AF_0_F_50_'+var)
    output = "models/AF_0_F_50_"+var+"/ML_results_AF_0_F_50_"+var+"_.csv"
